<script lang="ts">
	...
	async function backupList(tenant) {
		const response = await fetch('https://mark8t.ca/api/list.php' + '?tenant_id=' + tenant, {
			method: 'GET'
		});
...
<p>
	<small>
		This page is for managing backups. You can create a new backup, restore an existing backup,
		rename a backup, or delete a backup.
	</small>
</p>
<hr />
...
<button on:click={handleBackupClick} disabled={isLoading}>{isLoading ? '...' : 'Backup'}</button>
<button on:click={handleRestoreClick} disabled={isLoading || restoreDisabled}>Restore</button>
