<script>
	...
	//TODO READ FROM CORE
	export const _TENANT = import.meta.env.VITE_TENANT_ID.replaceAll('-', '_');
	...
	<Content id="fullscreen-content">
		<Card>
			<PrimaryAction on:click={() => clicked++}>
				...
				<Button
					on:click={() => {
						data.thumb = 'http://dundasandsons.ca/dev/_api/logo_black.jpg';
					}}
				>
				...
				<Content class="mdc-typography--body2">
					...
					<Components.Web.Static.Chip bind:keyed={data.tags} />
				</Content>
			</PrimaryAction>
			<Actions>
				<ActionButtons>
					...
					>
						...
						/>
						<input id="tenant" name="tenant" value={_TENANT} style="display:none" />
						<input
							type="file"
							name="fileToUpload"
							id="fileToUpload"
							on:change={(e) => {
								...
								document.getElementById('fileToUploadSubmit').click();
								console.log(_TENANT);
								data.thumb = _API + '/' + _TENANT + '/images/' + e.target.files[0].name;
								setTimeout(() => {
									let temp = data.thumb;
									data.thumb = '';
									data.thumb = temp;
