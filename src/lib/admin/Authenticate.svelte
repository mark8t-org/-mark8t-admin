<script>
	...
	const authenticate = async () => {
		console.log('Authenticate - secret - ' + localAccountHasAdminPermissions);
		checkSecret();
		console.log(
			'Authenticate - localAccountHasAdminPermissions - ' + localAccountHasAdminPermissions
		);
		...
		async function selectAccount() {
			...
			let msg = '';
			console.log('Authenticate - selectAccount - ', currentAccounts.length);
		...
		async function querySiteSpecificData() {
			Stores.Account.set(currentAccounts[0]);
		...
		function handleResponse(response) {
			if (response !== null) {
				Stores.Account.set(response.account);
		...
		function signIn() {
			...
			const isValidAccount =
				account && account.homeAccountId && account.localAccountId && account.username;

			if (isValidAccount) {
				console.log('Authenticate - signIn - account is valid');
				const timeToLive =
					new Date(Number(String(account.idTokenClaims.exp) + '000')) - new Date().getTime();

				const isValidToken = timeToLive > 0 && timeToLive < 1000 * 60 * 60 * 24 * 30;
				if (isValidToken) {
					console.log('Authenticate - signIn - account is valid - token is valid');
					console.log('Authenticate - signIn - time to live', timeToLive);
					return;
				}
			}
			...
			if (currentAccounts.length > 0) {
				localStorage.setObject('--accountId', currentAccounts[0], 24 * 60);
